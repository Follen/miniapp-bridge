...
| WMPF 私有 context 消息 + CDP Runtime context 事件 | `add/remove/connectJsContext`、`Runtime.executionContextCreated/Destroyed/ContextsCleared` | CDP 路由 context ID | 注册/选择/删除/清空；数字 ID 原样转十进制字符串 | 有选中项时路由其 `jscontext_id`，首次发现前发送空值 | connect 未知 ID 自动建立；stale generation 丢弃；断线按稳定顺序发布完整 removal | `internal/context`, `internal/app`, `sdk` | 私有路径 integration + SDK WebSocket bootstrap/destroy + App clear/disconnect observer | 已实现/已验证 |
