...
func TestGitHubCIWorkflowContract(t *testing.T) {
	...
	for _, required := range []string{
		...
		"git ls-files -z -- '*.go'",
		"gofmt -l \"${go_files[@]}\"",
		"go run github.com/rhysd/actionlint/cmd/actionlint@v${ACTIONLINT_VERSION} -ignore 'unexpected key \"queue\" for \"concurrency\" section'",
	...
	actionlintGate := strings.Index(workflow, "go run github.com/rhysd/actionlint/cmd/actionlint@v${ACTIONLINT_VERSION} -ignore 'unexpected key \"queue\" for \"concurrency\" section'")
