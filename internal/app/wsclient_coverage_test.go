...
func TestWSClientSendClosedChecksAndDeadlineFailure(t *testing.T) {
	...
	if err := client.sendLocked([]byte("locked-check")); !errors.Is(err, ErrClosed) {
		t.Fatalf("locked closed check error=%v", err)
	}

	deadlineErr := errors.New("set write deadline")
	client.closed.Store(false)
	connection.deadlineErr = deadlineErr
	if err := client.Send([]byte("deadline")); !errors.Is(err, deadlineErr) {
	...
	if err := client.Send([]byte("success")); err != nil {
		t.Fatal(err)
	}
